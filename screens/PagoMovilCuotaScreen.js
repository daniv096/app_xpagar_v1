...
const PagoMovilCuotaScreen = () => {
  const navigation = useNavigation();
  const route = useRoute(); 
  // Recibimos 'userId' y el objeto 'installment' de la cuota a pagar
  const { userId, installment } = route.params || {}; 

  // Estados para los campos del formulario
  const [amount, setAmount] = useState(''); // Monto de la cuota, no editable
  ...
  useEffect(() => {
    if (installment?.payment_amount) {
      // Pre-llenar y bloquear el monto con el valor de la cuota
      setAmount(parseFloat(installment.payment_amount).toFixed(2));
    } else {
      // Si no hay datos de cuota, alertar y regresar
  ...
  const handleConfirmPayment = () => {
    if (!amount || parseFloat(amount) <= 0) {
      Alert.alert("Error", "El monto de la cuota no es válido.");
      return;
    }
    ...
    if (!installment || !installment.payment_id) {
        Alert.alert("Error", "Información de la cuota no disponible para procesar el pago.");
        return;
    }
  ...
  // Manejador para el botón "Confirmar" dentro del modal (Pago de Cuota)
  ...
  const handleResultModalClose = () => {
    ...
    // Después de un pago de cuota, volvemos a la pantalla de AvanceEfectivoScreen para refrescar
    navigation.goBack(); 
  };
    ...
    >
      ...
      <SafeAreaView style={appStyles.safeArea}>
        <ScrollView contentContainerStyle={styles.scrollViewContent}>
          ...
          <Text style={styles.subtitle}>Completa los datos para pagar tu cuota vía Pago Móvil Directo</Text>
          ...
          <View style={styles.formContainer}>
            {/* Monto a Pagar (No editable) */}
            <Text style={styles.inputLabel}>Monto a Pagar</Text>
            <TextInput
              style={styles.input}
              value={amount}
              editable={false} // Siempre no editable para pago de cuotas
            />
            ...
            <TouchableOpacity style={styles.confirmButton} onPress={handleConfirmPayment}>
              <Text style={styles.confirmButtonText}>Pagar Cuota</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </SafeAreaView>
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalBody}>
              <Text style={styles.modalMessage}>
                Monto a pagar: <Text style={styles.modalHighlightText}>${parseFloat(amount || 0).toFixed(2)}</Text>
              </Text>
              <Text style={styles.modalMessage}>
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={[styles.modalTitle, { color: resultSuccess ? appColors.green : appColors.red }]}>
                {resultSuccess ? '¡Pago Exitosa!' : 'Error en la Operación'}
              </Text>
...
export default PagoMovilCuotaScreen;